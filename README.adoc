...
|`ghcr.io/teragrep/ipa_01/client/8:latest`
|`rockylinux/rockylinux:8`

|`ghcr.io/teragrep/ipa_01/client/9:latest`
|`rockylinux/rockylinux:9`
|===

The containers can be pinned to certainl releases of `:latest`, for example with `:1.0.0`.
...
No support for persistence, every container starts from after restart.
...
Any executable `.sh` files placed directly under `/ipa.conf.d/pre/` and `/ipa.conf.d/post/` directories will be executed before/after `ipa-{client,server}-install` commands respectively. It is recommended to prefix the scripts with numbers to ensure expected execution order, for example naming them `01_execute_this_first.sh`, `02_this_next.sh` and `99_do_this_last.sh`.
